...
public class LeadController {
    ...
    @GetMapping()
    ResponseEntity<PagedModel<LeadFullDto>> listAllLeads(@PageableDefault Pageable pageable) {
        return ResponseEntity.ok(leadService.findAllByTenantId(pageable));
    }

    @GetMapping("/name/{name}")
    ResponseEntity<PagedModel<LeadFullDto>> listAllLeadsByName(@PathVariable String name, @PageableDefault Pageable pageable) {
        return ResponseEntity.ok(leadService.listByNameLikeIgnoreCase(name, pageable));
    }

    @GetMapping("/{id}")
    ResponseEntity<ResponsePayload<LeadFullDto>> findById(@PathVariable UUID id) {
        LeadFullDto lead = leadService.findById(id);
        return ServiceUtils.createResponse(HttpStatus.OK, lead.id(), lead, messageUtils.getMessage("leads.founded"));
    }
}
